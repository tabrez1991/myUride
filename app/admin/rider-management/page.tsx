...
const RiderManagement = () => {
	...
	const handleAccept = async () => {
		setLoader(true)
		try {
			const result = await deleteRider(deletedDetails.email);
			const { data, error } = result;
			if (data) {
				setRender(!render)
				setLoader(false);
				handleDenied();
				handleAlert(data.userId, "Rider Deactivated Successfully", SUCCESS);
			} else {
	...
	const handleDeleteRider = (data: any) => {
	...
	const handleActivate = async (activeDetails: any) => {
		setLoader(true)
		try {
			const result = await activateRider(activeDetails?.email);
			const { data, error } = result;
			if (data) {
				setRender(!render)
				setLoader(false);
				handleDenied();
				handleAlert(data.userId, "Rider Activated Successfully", SUCCESS);
			} else {
				handleAlert(error.statusCode, error.message, ERROR);
				handleLogout()
			}
	...
	const options = [
		{
			label: "Edit Rider",
			handler: handleEditRider
		},
		{
			label: "Deactivate Rider",
			handler: handleDeleteRider
		}
	]
	...
	const columns: GridColDef[] = [
		{ field: 'riderId', headerName: 'Rider Id', width: 250 },
		{
			field: 'rider',
	...
	const getRidersDetails = async () => {
		setLoader(true)
		try {
			const tempRow: any = [];
			const result = await getRiders(page, pageSize, searchValue);
			const { data, error } = result;
			if (data) {
				data.forEach((element: any, index: number) => {
					tempRow.push({
						id: index + 1,
						riderId: element?.profile_id,
						rider: element?.fullname,
						// rider: element?.backgroundCheck?.legal_first_name,
